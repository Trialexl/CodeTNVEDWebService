...
model = AutoModelForSequenceClassification.from_pretrained(
    model_name, num_labels=660
).to(device)
Label_encoder = preprocessing.LabelEncoder()
Label_encoder.classes_ = np.load("codetnved/src/cl_classes2306.npy", allow_pickle=True)
model.load_state_dict(torch.load("pytorch_model.bin", map_location=device))

model_10d = AutoModelForSequenceClassification.from_pretrained(
    model_name, num_labels=2648
).to(device)
Label_encoder_10d = preprocessing.LabelEncoder()
Label_encoder_10d.classes_ = np.load(
    "codetnved/src/cl_classes1307.npy", allow_pickle=True
)
...
dscr_10d = pd.read_csv(
    "codetnved/data/descr10d.csv",
    sep=";",
    names=["id", "label", "till"],
    dtype={"id": str, "label": str, "till": str},
)
...
def predict_prob(text, qtty=2):
    ...
    inputs = tokenizer(
        text, truncation=True, max_length=100, padding="max_length", return_tensors="pt"
    )
    with torch.no_grad():
...
def getCodeOuterService(text):
    ...
    payload = {}
    headers = {"Cookie": "PHPSESSID=7NoMuGpXzqaSNjIakhZNcAtrik8lSKDm"}
    response = requests.request("GET", url, headers=headers, data=payload)
    return response.json()


def FixDataIn1c(answer):
    basic = HTTPBasicAuth(user1c, pwd1c)
...
def GetCode():
    ...
    if "qty" not in data:
        ...
        groups = predict_prob_with_descr(data["text"])
    else:
        codes = predict_prob_with_descr_10d(data["text"], data["qty"])
        groups = predict_prob_with_descr(data["text"], data["qty"])
